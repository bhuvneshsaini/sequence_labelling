...
def replace_entities_original_form(items, dict_for_original):
    ...
    for item in itemst:
        ...
        #print(item)
        if item !="":
            items=item+'\n' ## adding '\n' at the end or readable form because in dictionary '\n' added
            if items not in dict_for_original.keys():
                item_list.append(item)
            if items in dict_for_original.keys():
                new_item=items.replace(item,str(dict_for_original[items]))
...
def replace_entities(item, dict_for_readable):
    ...
    for items in rep_list:
        rep_item=items.split()
        for single_item in rep_item:
            print('single_item...........',single_item)
            if single_item !="":
                if single_item not in dict_for_readable.keys():
                    print("ssssssssss1",single_item)
                    item_list.append(single_item)
                if single_item in dict_for_readable.keys():             
                    new_item=single_item.replace(single_item,str(dict_for_readable[single_item]))
                    print("sssssssss.....",new_item)
                    new_items=new_item.replace('[','')             
                    new_items=new_items.replace(']','')
                    new_items = new_items.split('\\n')
                    new_items = "".join(new_items)               
